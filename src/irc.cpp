...
// Copyright (c) 2011-2012 FrogCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FrogCoinTEST0\r");
            Send(hSocket, "WHO #FrogCoinTEST0\r");
        } else {
            // randomly join #FrogCoin00-#FrogCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #FrogCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FrogCoin%02d\r", channel_number).c_str());
        }
